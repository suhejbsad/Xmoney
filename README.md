...
/* Xmoney Title */
.title {
font-size:80px; font-weight:800; position:relative; opacity:0; animation:appearStars 2s forwards; letter-spacing:5px;
}
...
/* VIP Card */
.vip-card {
margin:80px auto; background:rgba(255,255,255,0.05); padding:40px; width:90%; max-width:400px; border-radius:15px;
box-shadow:0 0 30px rgba(0,0,0,0.8); transition:0.4s; backdrop-filter:blur(5px);
}
...
// Palm Trees
let palms=[{x:canvas.width*0.25, y:canvas.height-150, sway:0.02, angle:0},
           {x:canvas.width*0.75, y:canvas.height-150, sway:0.015, angle:0}];
...
function animate(){
    ...
    // Draw palms
    palms.forEach(p=>{
        p.angle=Math.sin(Date.now()*p.sway)*0.3; // sway effect
        ctx.save();
